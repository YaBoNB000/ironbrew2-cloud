...
def verify(vm_path: Path, final_path: Path | None, late_nil_output: Path | None = None) -> None:
    ...
    # The final thread-level GetFEnv call is executor-sensitive. It must be
    # protected and fall back to the already captured PrimitiveEnvironment so a
    # late nil result can never reach RawGet or become the payload Env.
    fallback = re.search(
        rf"local\s+({IDENT})\s*=\s*({IDENT})\s*;\s*"
        rf"local\s+({IDENT})\s*,\s*({IDENT})\s*=\s*(?P<late_call>{IDENT}\s*\(\s*{IDENT}\s*\))\s*;\s*"
        rf"if\s+\3\s+and\s+{IDENT}\s*\(\s*\4\s*\)\s*==\s*[^;]+then\s*\1\s*=\s*\4\s*;\s*end\s*;",
        code,
        re.S,
    )
    if not fallback:
    ...
    if late_nil_output:
        call_start, call_end = fallback.span("late_call")
        instrumented = source[:call_start] + "true,nil" + source[call_end:]
        late_nil_output.write_text(instrumented, "latin1")

    print("PASS global no-op wiring: targets=getgenv/_G/thread, keys=char-derived, aliases=deduplicated, late-getfenv=fallback")
...
def main() -> int:
    ...
    parser.add_argument("--late-nil-output", type=Path)
    args = parser.parse_args()
    try:
        verify(args.generated_vm, args.generated_output, args.late_nil_output)
