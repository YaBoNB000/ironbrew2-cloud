...
def verify(vm_path: Path, final_path: Path | None) -> None:
    ...
    if len(re.findall(rf"\b{re.escape(validator_name)}\s*\(", code)) - 1 < 1:
        raise ValueError("no live CALL/TAILCALL leaf uses the dynamic-loader validator")
    ...
    if "NeedsDynamicCallGuard" not in generator or "GuardValidateCallTarget(Stk[Inst[OP_A]])" not in generator:
        raise ValueError("generator does not prepend validation to CALL/TAILCALL handlers")
    ...
    print("PASS per-call loadstring guard: identity/provenance/compile/run/constants, guarded CALL/TAILCALL leaves")
