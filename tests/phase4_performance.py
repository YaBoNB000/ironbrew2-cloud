...
def main() -> int:
    ...
    try:
        _duration, baseline_output = timed([args.lua, str(fixture)], root, 20)
        ...
        timed([args.lua, str(fixture)], root, 20, baseline_output)
        timed([args.lua, str(executor), "trusted", str(args.payload_output)], root, 20, baseline_output)
        for _ in range(5):
            baseline.append(timed([args.lua, str(fixture)], root, 20, baseline_output)[0])
        for _ in range(3):
