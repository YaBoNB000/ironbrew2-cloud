...
def verify(generated: Path, generated_vm: Path | None) -> None:
    ...
    if generated_vm:
        ...
        leaked = re.search(
            r"\b(?:StringShardState|StringParts|Shard(?:Count|Order|Index|Offset|Length|Position|Byte)|ExpectedShardLength)\b",
            code + "\n" + final_code,
        )
        if leaked:
    ...
    print(
        ...
        f"strings={strings}, multi-shard={multi_shard}, shard-counts={sorted(shard_counts)}, "
        "outer-unmask=inner-ciphertext"
    )
