...
The CLI has one supported configuration. Control-flow processing, bounded short SuperOperators, DEFLATE, VM-integrated AntiDump and strict `EnvironmentLock` executor attestation are enabled. Mutation, mega operators and destructive executor hooks remain disabled. Use `--line-info` only when original-line error reporting is required.
