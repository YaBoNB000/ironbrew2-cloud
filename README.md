...
- Linux differential suite: [`tests/run_linux_tests.sh`](tests/run_linux_tests.sh). Set `IB2_RANDOM_RUNS=20` for the full randomized matrix; it includes normal Luau capability simulation, wrapped string/raw/debug primitives, active hooks, contradictory closure classifiers, all three guard checkpoints, guard-name randomization, executor-global preservation, ephemeral block-cache probes, automatic dispatcher selection, safe fallback, route-state tamper rejection, Closure/SETLIST boundaries, and Lua 5.1 semantic checks.
