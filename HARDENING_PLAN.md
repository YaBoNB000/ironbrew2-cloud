...
- [~] M6：constant capsule tag 已从 `hash*31+byte` 改为 keyed two-lane cross-coupled authenticator；prototype tag、block manifest tag 与 instruction digest 仍待迁移，tag 宽度目前仍为 32-bit。
