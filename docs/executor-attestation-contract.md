...
| Prototypes | Inactive protos are inspectable, non-callable handles represented as either Luau functions or userdata; `getproto(..., true)` must still return active Luau functions that execute correctly. | The security property is that inactive protos expose their constants but cannot execute. Function-vs-userdata is an implementation representation, while callable inactive protos remain explicitly rejected. |
