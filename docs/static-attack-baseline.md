...
The harness recovers fused widths but stops before solving each build's shuffled member-token state machines and assigning Lua semantic names to every canonical handler or member operation. The final wrapper still contains the randomized fusion handler and shared-fragment implementations, so a sufficiently capable static analyzer can continue with control/data-flow classification.
