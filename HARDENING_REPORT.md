...
| thread / `_G` / `getgenv()` 的 `print`、`error`、`warn` 永久同一 no-op；其他 provider 不修改 | 通过 |
...
| vararg 与多返回值 | 通过 |
| table 与全局调用 | 通过 |
| payload `error()` 抑制；VM 私有 line-info runtime fault 定位 | 通过，payload error 为 no-op，私有错误路径包含原始 line 3 |
...
3. root prototype 的 schema、opaque constant capsules 和 block framing 仍在启动时恢复，但常量值只在引用它的完整 block manifest 认证后、实际 handler operand 首次读取时进入 invocation-local cache。字符串进一步拆为 1–7 个 prototype-keyed shards，按逻辑列分配字节、物理顺序 permutation 并各自 rolling-mask；outer capsule unmask 不再直接得到连续明文。 每个后续字符串的 inner state 还顺序吸收该 block 前序 capsule 的完整 authenticated ciphertext；静态工具必须恢复 block reference order 并重放整条 constant chain，不能独立打开单个 later capsule。 每个 constant operand use 还会分配独立随机 uint16 handle/capsule；重复字符串的不同使用点不共享 prototype constant index，静态工具必须分别恢复每个 use handle。 SETTABLE 的 key/value 不再以内联赋值表达式同时出现：四种 register/constant 组合使用随机 32-bit mode token，独立 key/value acquisition fragment 与随机求值顺序，最后才进入 shared commit fragment。 对 NEWTABLE 后连续、同 target、不同 constant key 且无跳转引用的安全写入组，IR 顺序还会在 opcode mapping 前随机 shuffle；可能有 alias/metatable/重复 key 的一般写入不重排。 最终 setter 还通过第二组 4 个随机 32-bit token 路由到 4 种只执行一次普通赋值的等价 leaf；operand mode 与 concrete commit leaf 不再是一层映射。 对已证明 fresh/unaliased 的 constructor writes，descriptor 额外携带 fresh flag；writer 使用不可与字符串 key 冲突的 table-object 临时 key，经 setter trampoline 插入 decoy、执行真实写入并在返回前清除。此类 instruction 被设为 fusion barrier，普通/可能带 metatable 的 table 不使用 decoy。默认 AntiDump 模式不会在共享 Chunk 表累积明文常量或指令；不过 capsule、索引和恢复算法都留在客户端，分析者仍可 hook capsule decode 或逐块触发执行来收集值。
...
7. Mutation 仍关闭；IR-native superoperator 已启用并覆盖 CFG、CLOSURE/CLOSE、SETLIST/data word 与 variable-return 安全边界；融合 handler 的成员不再按源码顺序线性排布，而是使用全 Build 唯一 member token、随机 branch order 与 bounded state machine 连接 supplemental operand semantics，10-build/CI 随机语义矩阵验证 physical lowering。
