...
-- Environment canaries and the mutated test upvalue are restored even after a
-- failed challenge. Payload-specific manifest/route/seed values do not exist in
-- a standalone script; equivalent transcript, token, state and seal arithmetic
-- is checked below with fixed diagnostic values.
...
local function record(name, passed, detail)
    if passed then
        ...
        Output0("[会触发静默 sink] " .. name .. ": "
            .. safe_text(detail or "condition returned false"))
    end
    return passed
end
...
Output0("逐项汇总: 不会触发静默 sink: " .. successCount
    .. " 会触发静默 sink: " .. failureCount)
if failureCount == 0 then
    Output0("综合结论: 当前环境不会因上述 executor gate 条件进入静默 sink")
else
    Output0("综合结论: 当前环境会进入静默 sink")
end
